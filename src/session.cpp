...
namespace tinymq
{
    ...
    void Session::process_packet(const Packet &packet)
    {
        ...
        default:
            ui::print_message("Session", "Received unsupported packet type: " + std::to_string(static_cast<int>(packet.type())), ui::MessageType::WARNING);
            break;
        }
    ...
    void Session::handle_admin_request(const Packet &packet)
    {
        if (!is_authenticated_)
        {
            ui::print_message("Session", "Unauthenticated client trying to make admin request", ui::MessageType::WARNING);
            return;
        }
        ...
        if (!payload.empty())
        {
            try
            {
                ...
                if (json.contains("action") && json.contains("topic"))
                {
            ...
            {
                ui::print_message("Session", "Error processing admin request: " + std::string(e.what()), ui::MessageType::ERROR);
            }

            send_ack(PacketType::ADMIN_REQ);
        }
    }
